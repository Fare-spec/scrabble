...
class ScrabbleGUI:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.tile_font = tkfont.Font(family="Courier", size=16)
        self.ui_font = tkfont.Font(family="Segoe UI", size=12)
    ...
    def _build_widgets(self) -> None:
        ...
        self.board_frame.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=(0, 5))

        # chaque ligne/colonne du plateau peut s'étirer
        for i in range(15):
        ...
        main_frame.columnconfigure(1, weight=1, minsize=260)
    ...
    def _on_resize(self, event=None) -> None:
        ...
        bw = self.board_frame.winfo_width()
        bh = self.board_frame.winfo_height()
        if bw <= 0 or bh <= 0:
            return

        # taille d'une case en pixels
        cell_size = min(bw, bh) // 15
        if cell_size <= 0:
            return

        # facteur pour la taille de police (à ajuster à ton goût)
        new_font_size = max(8, cell_size - 4)

        if new_font_size != self.tile_font.cget("size"):
            self.tile_font.configure(size=new_font_size)
